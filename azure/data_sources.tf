locals {
    azure_rg        = lower("${var.prefix}-${local.env}-${var.rg_name}-${local.deployment_id}")
    storage_name    = lower("${var.prefix}${local.env}${local.deployment_id}")
    public_ip       = lower("${var.prefix}-${local.env}-publicIP-${local.deployment_id}")
